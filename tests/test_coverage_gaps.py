...
class TestMatchFunction:
    """Test match function (lines 217-226)."""

    def test_match_empty_line_pattern_on_empty_line(self):
        """Match empty-line pattern on actual empty line."""
        result = match(EMPTY_LINE_PATTERN, "")
        assert result is True

    def test_match_empty_line_pattern_on_non_empty(self):
...
class TestFormatWhitespace:
    """Test whitespace formatting (lines 661-675)."""

    def test_format_whitespace_spaces(self):
        """Spaces are converted to middle dots."""
        assert _format_whitespace("    ") == "····"

    def test_format_whitespace_tabs(self):
        """Tabs are converted to right arrows."""
        assert _format_whitespace("\t\t") == "→→"

    def test_format_whitespace_mixed(self):
        """Mixed whitespace formatting."""
        assert _format_whitespace(" \t ") == "·→·"

    def test_format_whitespace_empty(self):
        """Empty string."""
        assert _format_whitespace("") == ""

    def test_format_whitespace_other(self):
        """Other characters pass through."""
        assert _format_whitespace("x") == "x"


class TestPatternLines:
    """Test pattern_lines utility (lines 678-680)."""

    def test_pattern_lines_simple(self):
        """Simple pattern split."""
        assert pattern_lines("line1\nline2") == ["line1", "line2"]

    def test_pattern_lines_empty_lines_filtered(self):
        """Empty lines are filtered out."""
        assert pattern_lines("line1\n\nline2") == ["line1", "line2"]

    def test_pattern_lines_trailing_newline(self):
        """Trailing newline is handled."""
        assert pattern_lines("line1\nline2\n") == ["line1", "line2"]

    def test_pattern_lines_empty_string(self):
        """Empty string returns empty list."""
        assert pattern_lines("") == []

    def test_pattern_lines_only_empty_lines(self):
        """Only empty lines returns empty list."""
        assert pattern_lines("\n\n\n") == []


class TestLine:
    """Test Line class (lines 229-244)."""
    ...
    def test_line_matches_exact(self):
        """Exact match."""
        line = Line("hello")
        assert line.matches("hello") is True

    def test_line_matches_wildcard(self):
        """Wildcard match."""
        line = Line("hello world")
        assert line.matches("...world...") is True

    def test_line_matches_no_match(self):
        """No match."""
        line = Line("hello")
        assert line.matches("goodbye") is False

    def test_line_mark_upgrade(self):
    ...
    def test_line_mark_no_downgrade(self):
    ...
    def test_line_mark_same_level(self):
...
class TestAuditMethods:
    """Test Audit class methods (lines 247-358)."""
    ...
    def test_audit_optional_simple(self):
    ...
    def test_audit_refused_no_match(self):
    ...
    def test_audit_continuous_simple(self):
    ...
    def test_audit_continuous_with_empty_lines(self):
    ...
    def test_audit_continuous_broken(self):
    ...
    def test_audit_continuous_incomplete(self):
    ...
    def test_audit_in_order_simple(self):
    ...
    def test_audit_in_order_out_of_order(self):
        """In-order pattern with wrong order."""
        audit = Audit("b\na\nc")
        audit.in_order("pattern1", ["a", "b"])
        assert len(audit.unmatched_expectations) > 0

    def test_audit_in_order_partial_reset(self):
...
class TestAuditBuildSummary:
    """Test Audit._build_summary method (lines 359-423)."""

    def test_build_summary_single_pattern(self):
    ...
    def test_build_summary_multiple_patterns(self):
    ...
    def test_build_summary_refused(self):
...
class TestAuditCollectWhitespaceIssues:
    """Test Audit._collect_whitespace_issues (lines 486-508)."""

    def test_collect_whitespace_unexpected_trailing(self):
        """Collect trailing whitespace from unexpected lines."""
        audit = Audit("line  ")
        audit.in_order(
            "pattern1", ["line"]
        )  # Won't match due to trailing space
        issues = audit._collect_whitespace_issues()
        assert len(issues) > 0
        assert issues[0][0] == 1  # Line number
        assert "trailing" in issues[0][1]

    def test_collect_whitespace_unexpected_only_whitespace(self):
    ...
    def test_collect_whitespace_unmatched_expected(self):
...
class TestAuditBuildContext:
    """Test Audit._build_context method (lines 518-534)."""

    def test_build_context_middle(self):
    ...
    def test_build_context_beginning(self):
    ...
    def test_build_context_end(self):
...
class TestAuditToJson:
    """Test Audit.to_json method (lines 536-579)."""
    ...
    def test_to_json_unmatched_patterns(self):
    ...
    def test_to_json_matched_refused(self):
...
class TestAuditBuildUnmatchedEntry:
    """Test Audit._build_unmatched_entry (lines 581-600)."""

    def test_build_unmatched_entry_basic(self):
        """Basic unmatched entry."""
        audit = Audit("line1\nline2")
        entry = audit._build_unmatched_entry(
            "pattern1", "missing", is_primary=True
        )
        assert entry["pattern"] == "pattern1"
        assert entry["expected_line"] == "missing"
        assert entry["failure_type"] == "primary"

    def test_build_unmatched_entry_with_position(self):
    ...
    def test_build_unmatched_entry_cascading(self):
...
class TestAuditBuildMatchedRefusedEntry:
    """Test Audit._build_matched_refused_entry (lines 602-618)."""

    def test_build_matched_refused_entry_basic(self):
...
class TestFormatLineReport:
    """Test format_line_report function (lines 621-658)."""

    def test_format_line_report_expected(self):
    ...
    def test_format_line_report_optional(self):
    ...
    def test_format_line_report_unexpected(self):
        """Format unexpected line."""
        result = format_line_report(Status.UNEXPECTED, "🟡", "", "line content")
        assert "🟡" in result
        assert "line content" in result

    def test_format_line_report_with_trailing_whitespace(self):
    ...
    def test_format_line_report_with_trailing_whitespace_no_color(self):
    ...
    def test_format_line_report_whitespace_only(self):
    ...
    def test_format_line_report_with_tabs(self):
    ...
    def test_format_line_report_long_cause(self):
    ...
    def test_format_line_report_no_original_line(self):
...
class TestPattern:
    """Test Pattern class (lines 683-769)."""
    ...
    def test_pattern_continuous(self):
    ...
    def test_pattern_in_order(self):
    ...
    def test_pattern_normalize(self):
    ...
    def test_pattern_flat_ops(self):
    ...
    def test_pattern_generate_example_simple(self):
    ...
    def test_pattern_generate_example_with_wildcards(self):
    ...
    def test_pattern_generate_example_empty_line(self):
    ...
    def test_pattern_generate_example_ignores_refused(self):
    ...
    def test_pattern_replace_wildcards_ellipsis(self):
    ...
    def test_pattern_replace_wildcards_empty_line(self):
    ...
    def test_pattern_replace_wildcards_normal(self):
    ...
    def test_pattern_eq_success(self):
    ...
    def test_pattern_eq_failure(self):
    ...
    def test_pattern_eq_with_other_object(self):
...
class TestPatternsLib:
    """Test PatternsLib class (lines 771-773)."""

    def test_patterns_lib_getattr(self):
    ...
    def test_patterns_lib_caching(self):
    ...
    def test_patterns_lib_multiple_patterns(self):
...
class TestShouldUseColor:
    """Test _should_use_color function (lines 36-49)."""

    def test_should_use_color_no_color_env(self, monkeypatch):
    ...
    def test_should_use_color_flag(self):
...
class TestAuditReport:
    """Test Audit.report method (lines 425-484)."""
    ...
    def test_report_with_refused(self):
    ...
    def test_report_with_whitespace_issues(self):
...
class TestMultiPatternSummary:
    """Test multi-pattern summary building (lines 402-403, 410-411)."""

    def test_build_summary_refused_multiple_patterns(self):
    ...
    def test_build_summary_unmatched_multiple_patterns(self):
...
class TestJsonContextBranches:
    """Test JSON context building branches (lines 611-618, 613-615)."""

    def test_build_matched_refused_entry_with_position_in_range(self):
    ...
    def test_build_matched_refused_entry_position_out_of_range(self):
    ...
    def test_build_unmatched_entry_position_out_of_range(self):
...
class TestContinuousEdgeCases:
    """Test continuous method edge cases (line 344->330)."""

    def test_continuous_first_line_no_match(self):
...
class TestMatchEdgeCases:
    """Test match function edge cases (lines 219-220)."""

    def test_match_empty_line_marker_on_empty_line(self):
        """Match <empty-line> marker on empty line."""
        result = match(EMPTY_LINE_PATTERN, "")
        assert result is True

    def test_match_empty_line_marker_on_content(self):
